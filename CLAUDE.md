...
**OPEN DECISION: generated claims are not under that rule, and one of them is at 0.8pp.** The tens-floor bakes `floor10(worse cohort)` into the sentence, so by construction it leaves between 0 and 10pp of headroom. `ssd-listing-both-up` is currently at **0.8pp** (the /ssd/ floor is 130% against NVMe 1y at +130.8% full). It self-corrects at the next regen, which is the argument for leaving it, and a 1pp intraday move breaches a live sentence until then, which is the argument against. Extending the 25pp rule to generated floors needs a coarser step than tens, roughly `floor10(worse - 25)`, and that makes every generated magnitude noticeably more conservative than the data supports. Not decided.
