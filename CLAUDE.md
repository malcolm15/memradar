...
- **Price Analysis (`Price Analysis` block in the stats region):** a per-product natural-language paragraph assembled from the baked stats. Option-C freshness split: a BAKED historical narrative (`.pdp-analysis-history`: tracking span, N points, ATL/ATH values + dates, 30-day move) that never hydrates (crawlable SEO), plus a HYDRATED current-assessment span (`#pdpAnalysisCurrent`: current position vs low/high, $/GB, and the good/typical/elevated verdict). pdp-hydrate.js `recomputeAnalysis()` rebuilds the current span from the live price via `textContent`. The verdict and the buy-indicator share ONE predicate, `buyState()` (defined identically in the generator and pdp-hydrate.js, thresholds from `BUY_GOOD_MAX_RATIO`/`cfg.goodMaxRatio`), so they can never disagree. Honest-flag override: a good-buy price still >= `INFLATION_ATL_RATIO` (1.5) x the all-time low reads as "near its recent low but well above where it traded a year or two ago" (the memflation reality; ~143/235 products). $/GB uses the same `VALUE_LOW/HIGH_RATIO` as the value-metric line. No em dashes; all figures numbers/dates/enums (no user text). Regenerate to change.
...
- **Newegg Phase 2 — feed cron (design only, NOT built):** Newegg approval is live via Rakuten (Publisher SID 4705448, Newegg MID 44583). **Phase 1 SHIPPED (Aug 2026):** 115 `retailer_offers` rows live (98 tier-1 MPN, 17 Malcolm-approved tier-2; 120 products remain Amazon-only), two-retailer Buy Now rows + AggregateOffer JSON-LD on two-offer PDPs, Newegg/Rakuten copy pass across footer legal, FAQ, affiliate.html, terms, privacy. Phase 1 components: `retailer_offers` table (current-price comparison rows; clean URLs stored, Rakuten deep-link wrapping at render time via `backend/lib/rakutenLink.js`), `scripts/match-newegg.js` (tier-1 normalized-MPN auto-match + tier-2 conservative name proposals reusing the family-clustering normalization; human review gate before any write), and generator/hydration support for a second Buy Now row (cheapest in-stock first; no-op while `retailer_offers` is empty). Feed delivery is confirmed working: Rakuten SFTP (`scripts/fetch-newegg-feed.js`; complete file ~156 MB gz / 1.4 GB text / ~1.05M rows, positional 38-col pipe format, plus a small daily delta file `44583_4705448_mp_delta.txt.gz` ~1.6 MB that is the better cron fetch). Intended cron shape: fetch feed (delta preferred) → refresh `retailer_offers` price/in_stock/fetched_at where retailer='newegg' (matched rows only, NEVER re-match automatically) → PDPs hydrate from it. The complete feed contains ONLY in-stock items — disappearance from the feed is the out-of-stock signal, so the cron must handle absent rows (mark stale/OOS), and the fetch needs a completion watchdog (ssh2-sftp-client fastGet was observed hanging after the final byte on the big file). SCOPE GUARDRAIL: price history, charts, Price Analysis, buy verdict, and alerts remain Amazon/Keepa-only; Newegg is a current-price comparison row.
