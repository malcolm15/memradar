...
**Fix / convention:** a single shared version query is appended to **both `style.css` and every local JS include** on every page — `?v=YYYYMMDD` (current value: **`20260913`**). A new URL forces browsers to refetch immediately regardless of max-age.
...
**Mobile is TWO columns, not one**, and the `Site` column spans the full width flowing its own list in two CSS columns. One column would stack three headings and fifteen links into a tower, which is the thing the regrouping fixes; two columns without the span left `Site` alone on row two with an empty cell beside it. **The `@media (max-width: 768px)` override MUST sit after the base `.footer-nav` rule in source order** - it was first written into the 768px block at line 310, ahead of the base rule at 488, and lost outright because a media query adds no specificity. Same trap as `.pulse-windows`, second instance.
