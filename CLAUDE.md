...
**THE TWO FILES CARRY DIFFERENT ATTRIBUTION WORDING, AND THE DIFFERENCE IS THE LICENCE RATHER THAN STYLE (2026-09-21).** The market file's figures are MemRadar's own aggregate medians across products, described in its own payload as "not the raw licensed data", so inviting republication with credit is ours to invite and it keeps the line the CSV has always carried. The products file carries PER-PRODUCT history derived from licensed data, where consent 2 covers monthly points for use with the extension and says nothing about onward republication. Its attribution previously ended "Credit MemRadar and Keepa if you republish", which invited exactly what was not granted and reads as purporting to grant a sublicence, which **Section 18(7)(c)** prohibits. That sentence is gone; the products file now ends "This file is provided for use with the MemRadar Raycast extension; it does not carry a right to redistribute the price history." **The extension's pre-flight fails if the word "republish" ever reappears in the products payload**, so the two strings cannot be merged back together by a future tidy-up. Do not merge them.
