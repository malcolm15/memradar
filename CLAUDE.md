...
- **`sharp` is a PRODUCTION dependency (since 2026-09-19)** because the daily regen renders the chart images and every workflow installs with `npm ci --omit=dev`. `to-ico` and `puppeteer` stay dev-only. Measured cost on the runner, same-machine A/B: `npm ci --omit=dev` +0.1s warm cache (2.9s to 3.0s), +0.3s cold (3.1s to 3.4s), node_modules 14MB to 31MB, on both the regen and Newegg jobs. Moving it also corrected a stale lockfile root that listed `ssh2-sftp-client` under devDependencies; that was cosmetic, since npm recomputes dev flags from package.json and the Newegg job already installed it (verified on the runner).
