...
**TWO PRIVACY-POLICY CLAIMS ARE UNVERIFIED, and settling them is Malcolm's (both need dashboard access this project's credentials do not have).** A line-by-line check of `privacy.html` against shipped behaviour on 2026-09-19 found every other claim true, or fixed it. These two remain, recorded so they are not forgotten:
- **Supabase "hosted in the United States".** The API does not expose the project's region (its responses come through Cloudflare's edge, which says nothing about where the database is). **Settles it:** Supabase dashboard, Project Settings, General, Region. If it is not a US region, the policy's Supabase line changes.
- **Google Analytics: "anonymous page view data", "No personally identifiable information", "anonymous cookies".** The site's half is verified: the only custom event is `search` (`search_term`, `result_count`), no `user_id` is set, and no email or other form input is ever sent to GA. Google's half is not: the `_ga` cookie is a pseudonymous identifier, and what the property collects depends on its settings. **Settles it:** GA4 Admin, Data collection and modification (Google signals and granular location and device data collection OFF; user-provided data collection OFF), and Data retention. If any of those is on, "anonymous" overstates it.
