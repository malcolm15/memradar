...
**Back to top (`frontend/js/back-to-top.js`, listing pages only):** a circular floating button (bottom-right, safe-area-inset padding, 48px tap target, `aria-label="Back to top"`, real `<button>`) that smooth-scrolls to the top. Self-contained — it creates its own element, so the two listing pages just include the script (after `filter-sheet.js`); not on other pages yet. Visibility uses a `requestAnimationFrame`-throttled scroll read (`scrollY > 1.5 viewports`), chosen over an IntersectionObserver sentinel because a 1.5vh marker would add phantom scroll height on short filtered result sets. Respects `prefers-reduced-motion` (instant jump, no rise animation). z-index 90 — under the filter sheet (900) and alert modal (1000), so those overlays cover it. Dark-mode styled.
...
**Fix / convention:** a single shared version query is appended to **both `style.css` and every local JS include** on every page — `?v=YYYYMMDD` (current value: **`20260811`**). A new URL forces browsers to refetch immediately regardless of max-age.
