...
**Fix / convention:** a single shared version query is appended to **both `style.css` and every local JS include** on every page — `?v=YYYYMMDD` (current value: **`20260830`**). A new URL forces browsers to refetch immediately regardless of max-age.
