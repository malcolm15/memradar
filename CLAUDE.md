...
**Retailer-semantic colors (NOT palette additions).** `--retailer-amazon` / `--retailer-newegg` (plus `-text` / `-border` pairs) exist for ONE purpose: identifying a retailer button in the PDP header strip. They are never accents, never reused elsewhere; cobalt remains the site's only accent system, and the Buy Now table's buttons deliberately stay cobalt (the strip is the primary affordance, the table is the detail view).
...
**Fix / convention:** a single shared version query is appended to **both `style.css` and every local JS include** on every page — `?v=YYYYMMDD` (current value: **`20260824`**). A new URL forces browsers to refetch immediately regardless of max-age.
