...
**`stable_pct_change` is computed in-memory and never persisted.** The floors need the stable cohort's OWN signed figure, and the stored `stability_delta_pp` is `Math.abs()` — an unsigned distance cannot say which side of a floor the stable figure landed on. It is deliberately not a column: this is a monitoring layer, and adding one would put a pending ALTER TABLE between the floors and the run that needs them, which is exactly how the tripwire shipped disabled for a week.
